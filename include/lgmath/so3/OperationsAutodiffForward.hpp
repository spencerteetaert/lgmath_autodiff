...
/**
 * \file Operations.hpp
 * \brief Header file for the SO3 autodiff Lie Group math functions.
...
inline autodiff::real1st clamp(const autodiff::real1st& val,
                               const autodiff::real1st& min,
                               const autodiff::real1st& max) {
  return (val < min) ? min : (max < val) ? max : val;
}

inline autodiff::real1st smooth_acos(const autodiff::real1st& x) {
  if (1.0 - fabs(x) > 1e-6)
    return acos(x);
  else
